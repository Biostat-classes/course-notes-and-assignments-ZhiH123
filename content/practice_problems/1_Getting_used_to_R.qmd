...
The : means Gives the numbers in a list starting from the first value
(5) before the colon to the last value (15) after the colon, and it
...
This gives the number of items in the list, or how long the list is from
5 to 11.
...
This gives the maximum number in the list, which is 15

```{r}
x[x < 10]
```

This gives all the numbers that are less than 10

```{r}
x^2
```

This takes each number in the list and raise it to the power of 2, or
square it.
...
This gives the numbers that are greater than 7, but less than 12.
...
The first two give the same outputs where it gives a list of numbers
from -1 to 2, but the parentheses don't seem to change the command. The
third one gives a list from 1 to 2, which is just 1,2. but it applies
what's outside the parentheses to the output, so both numbers become
negative.
...
62 Rows, 62 is the number of rows and 12 is the number of columns, so
dim gives the dimension of the table.
